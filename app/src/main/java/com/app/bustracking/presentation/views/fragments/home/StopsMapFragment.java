...
//        List<Stop> stopsList = stopsDao.fetchAllStops(agencyId);


        //fetch data from db
//        int travelId = requireArguments().getInt(RoutesFragmentKt.ARGS);
//         route = routesDao.fetchRoute(travelId);
...
//        builder.include(new LatLng(Double.parseDouble(stop.getLat()), Double.parseDouble(stop.getLng())));
//        }
